...
//AUTOROLE MANAGER
const Experience = require('./../../../structures/user/Experience.js')
let userLevel = await Experience.getLevel(message.member)
const autoroles = guild.get("autoroles");
if (autoroles.roles.length && !timestamps.has(message.author.id)) autorole();

function autorole() {
  if (!autoroles.roles) return;
  autoroles.roles.map((level, i) => {
    let autoroleObject = autoroles.roles[i]
