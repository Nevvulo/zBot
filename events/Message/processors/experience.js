...
module.exports = async function experience(message, guild) {
    ...
    if (isExperience && !isIgnored) {
        ...
        const level_before = await Experience.getLevel(member)
        ...
        await member.update("experience", experience+rand) //increment
        const level_after = await Experience.getLevel(member)
        if (level_after > level_before) (levelMessages ? function(){
            ...
            message.channel.zend(message.guild.client.util.emoji("info", message.guild) + " Congratulations **" + message.author.username +
             "**, you leveled up!\n**LEVEL** `" + level_before + "` ─ `" + level_after + "`  |  `" + experience_to_next + "` **EXP** *to next level*")
        }() : "")

        if (experience+rand > 99999999999999 || experience > 99999999999999) {
            await member.update("experience", 99999999999999)
        }
    }
