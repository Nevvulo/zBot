...
var Settings = require('./structures/general/Settings.js');
...
async function setGame() {
  ...
  presence.activity.name = "https://zbot.me";
  ...
  switch (text) {
    ...
    default:
      if (text.startsWith("log")) {
        ...
        for (let i in recentErrors) {
          for (let i = 0; i < 3; i++) {
            logBox.log(previousMessages[i]);
          }
          logBox.log(recentErrors[i]);
        }
      } else if (text.startsWith("newguilds")) {
        client.guilds.forEach(function(i) {
          Settings.newGuild(i);
        });
      } else if (text.startsWith("initdb")) {
        const Config = JSON.parse(fs.readFileSync('./Settings.json', 'utf8'));
        client.guilds.forEach(async function(i) {
          Settings.newGuild(i);
        Object.entries(Config[i.id]).forEach(
            ([key, value]) => {

        Settings.editSetting(i, key, value);
        })
        });
        ...
        client.guilds.forEach(async function(i) {
          ...
          await Settings.checkGuild(i);
          await Settings.checkGuildSettings(i);
        });
      }
  }
}
...
client.on('guildMemberAdd', async function(guildMember) {
  if (client.channels.has(await Settings.getValue(guildMember.guild, "joinMessageChannel"))) {
    channel = client.channels.get(await Settings.getValue(guildMember.guild, "joinMessageChannel"));
  } else {
    return;
  }

  if (client.channels.has(await Settings.getValue(guildMember.guild, "memberLogsChannel"))) {
    memberchannel = client.channels.get(await Settings.getValue(guildMember.guild, "memberLogsChannel"));
  } else {
    return;
  }
  ...
  if (await Settings.getValue(guildMember.guild, "joinMessageEnabled")) {
    if (await Settings.getValue(guildMember.guild, "joinMessage") === "null" || await Settings.getValue(guildMember.guild, "joinMessage") === "none" || await Settings.getValue(guildMember.guild, "joinMessage") === "") {
      ...
      channel.send("​" + await Settings.getValue(guildMember.guild, "joinMessage").replace("{user}", guildMember).replace("{username}", guildMember.user.username));
    }
  }

  embed = new Discord.MessageEmbed();
  embed.setAuthor("ᴍᴇᴍʙᴇʀ ᴊᴏɪɴᴇᴅ »  " + guildMember.user.tag, guildMember.user.avatarURL({
    format: 'png'
  }));
...
client.on('guildMemberRemove', async function(guildMember) {
  ...
  embed = new Discord.MessageEmbed();
  embed.setAuthor("ᴍᴇᴍʙᴇʀ ǫᴜɪᴛ »  " + guildMember.user.tag, guildMember.user.avatarURL({
    format: 'png'
  }));
...
client.on('guildMemberUpdate', async function(oldMember, newMember) {
  if (client.channels.has(await Settings.getValue(newMember.guild, "memberLogsChannel"))) {
    channel = client.channels.get(await Settings.getValue(newMember.guild, "memberLogsChannel"));
  } else {
    return;
  }
  ...
  if (newMember.nickname != oldMember.nickname) {
    if (newMember.nickname == null) {
      embed = new Discord.MessageEmbed();
      embed.setAuthor("ɴɪᴄᴋɴᴀᴍᴇ ᴄʜᴀɴɢᴇ »  " + oldMember.user.tag, oldMember.user.displayAvatarURL());
      embed.setColor("#42b3f4");
      embed.setDescription(":label: <@" + oldMember.id + "> has reset their nickname. It has defaulted to their username.\n");
      ...
      embed.setAuthor("ɴɪᴄᴋɴᴀᴍᴇ ᴄʜᴀɴɢᴇ »  " + oldMember.user.tag, oldMember.user.displayAvatarURL());
      embed.setColor("#42b3f4");
      embed.setDescription(":label: <@" + oldMember.id + "> has changed their nickname.\n");
      embed.addField("**Previous Nickname**", oldMember.displayName);
...
client.on('messageDelete', async function(message) {
  ...
  if (message.content.startsWith(await Settings.getValue(message.guild, "prefix"))) return;
  if (message.author.bot) return;
  ...
  const embed = new Discord.MessageEmbed();
  embed.setAuthor("ᴍᴇꜱꜱᴀɢᴇ ᴅᴇʟᴇᴛᴇᴅ »  " + message.author.tag, message.author.avatarURL({
    format: 'png'
  }));
...
client.on('messageUpdate', async function(oldMessage, newMessage) {
  ...
  if (oldMessage.guild != null) {
    if (client.channels.has(await Settings.getValue(oldMessage.guild, "modLogsChannel"))) {
      channel = client.channels.get(await Settings.getValue(oldMessage.guild, "modLogsChannel"));
    } else {
      return;
    }
    ...
    if (channel != null) {
      const embed = new Discord.MessageEmbed();
      embed.setAuthor("ᴍᴇꜱꜱᴀɢᴇ ᴇᴅɪᴛᴇᴅ »  " + oldMessage.author.tag, oldMessage.author.avatarURL({
        format: 'png'
      }));
