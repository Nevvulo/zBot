...
function newMessage(message) {
    ...
    //CHALLENGE
    if (challenge.challengeInDMS == false) {
            ...
            if (msg == "attack" || msg == "defend" || msg == "heal" || msg == "end" || msg == "special") {
                ...
                if (msg.toLowerCase().startsWith("attack")) {
                    ...
                    if (miss != 4 || specialAbilityUsed[oppturn.id] == true) {
                        ...
                        message.channel.send(":crossed_swords: **CHALLENGE**: " + oppturn + " took **" + damage + "** damage. They now have **" + health[oppturn] + " HP**.")
                    } else {
                        message.channel.send(":crossed_swords: **CHALLENGE**: " + oppturn + " dodged " + turn + "'s attack!");
                    }
                ...
                if (msg.toLowerCase().startsWith("defend")) {
                    ...
                    message.channel.send(":crossed_swords: **CHALLENGE**: " + turn + " equiped some armor. They now take **" + defense + " HP** less damage for every attack.")
                ...
                if (msg.toLowerCase().startsWith("heal")) {
                    if (timesHealed[turn.id] < 4) {
                        ...
                        message.channel.send(":crossed_swords: **CHALLENGE**: " + turn + " drank a **level " + healLevel[turn] + "** health potion. They healed **" + healAmount + " HP**, bringing them up to **" + health[turn] + " HP**.");
                ...
                if (msg.toLowerCase().startsWith("special")) {
                    if (userChallenge.weapon == "diamond") {
                        if (specialAbilityUsed[turn.id] == false) {
                            ...
                            message.channel.send(":crossed_swords: **CHALLENGE**: " + turn + " used their special ability: **Diamond Frost**! " + turn + " gained **20 HP** and " + oppturn + " will miss their next turn.");
                        ...
                        if (specialAbilityUsed[turn.id] == false) {
                            ...
                            message.channel.send(":crossed_swords: **CHALLENGE**: " + turn + " used their special ability: **Cutting Edge**! The sharp, iron blade cuts " + oppturn + " badly, causing them to bleed. They will lose **5 HP** for every turn they take. ");
                ...
                if (msg.toLowerCase().startsWith("end")) {
                    if (confend) {
                        message.channel.send(":crossed_swords: **CHALLENGE**: The war between " + turn + " and " + oppturn + " has **ended**.");
                        end = true;
                        return;
                    } else {
                        confend = true;
                        message.channel.send(":crossed_swords: **CHALLENGE**: Are you sure you want to end this challenge? Type `end` again to confirm.");
                        return;
                    }
                }
