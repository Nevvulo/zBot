...
exports.run = (client, message, args) => {
	message.delete();
		...
		if (args == "" || args == undefined) {
			message.reply(':no_entry_sign: **ERROR:** You need to enter a user to mute. See `' + await Settings.getValue(message.guild, "prefix") +'help mute` for more information.');
			return;
		}

		message.guild.members.fetch(args.split(" ").toString()).then(async function (member) {
			if (!message.guild.roles.exists("name", await Settings.getValue(message.guild, "muteRole"))) {
