exports.settings = {command: "softban", description: "Softban a specified user from this server.", usage: "softban (user) [reason]", throttle: {usages: 2, duration: 10},
...
exports.run = async (client, message, args) => {
	...
	function punish(guild, type, moderator, member, reason, time) {
		...
		message.channel.zend(`${client.util.emoji("ok", message.guild)} \`${member.displayName}\` was successfully softbanned.`)
	}
...
this person. Make sure I have the \`${this.settings.permission.user}\` permission and try again.`)
	...
	if (await Settings.getValue(message.guild, "punishConfirmation")) {
		...
		.catch(collected => {
			if (!collected.size) return message.channel.zend(`${client.util.emoji("empty", message.guild)} The softban against **${member.displayName}** won't be completed because you didn't reply with an answer in time.`)
		})
		return message.channel.zend(`:gear: **${this.settings.command.toUpperCase()}**: Are you sure you want to issue this softban against **${member.displayName}**? *(__y__es | __n__o)*`, { embed })
	} else {
		punish(message.guild, this.settings.command, message.author, member, reason, null);
	}
}