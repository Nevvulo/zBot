exports.settings = {command: "warn", description: "Gives the specified user a warning. Warnings aren't destructive like bans or kicks, they simply serve as a reminder to warn the user that they did something wrong.",
	usage: "warn (user) [reason]", throttle: {usages: 2, duration: 10}, permission: {command: "mod", user: "MOVE_MEMBERS"} }
exports.run = async (client, message, args) => {
	...
	function punish(guild, type, moderator, member, reason, time) {
		...
		message.channel.zend(`${client.util.emoji("ok", message.guild)} \`${member.displayName}\` was successfully warned.`)
	}
	...
	if (await Settings.getValue(message.guild, "punishConfirmation")) {
		...
		.catch(collected => {
			if (!collected.size) return message.channel.zend(`${client.util.emoji("empty", message.guild)} The warning against **${member.displayName}** won't be completed because you didn't reply with an answer in time.`)
		})
		return message.channel.zend(`:gear: **${this.settings.command.toUpperCase()}**: Are you sure you want to give a warning to **${member.displayName}**? *(__y__es | __n__o)*`, { embed })
	} else {
		punish(message.guild, this.settings.command, message.author, member, reason, null);
	}
}